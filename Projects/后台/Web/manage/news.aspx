	...
	<div>
		...
		{ %>
					<table border="1" bordercolor="#eae3e4" cellpadding="0" cellspacing="0" class="tablecss1">
						...
						<%if (Convert.ToInt32(Request.QueryString["pid"]) == 196 | Convert.ToInt32(Request.QueryString["pid"]) == 223)
		{ %><tr <%=ShowField("NewsPhoto") %>>
			  <td class="tdMargin" style="width: 68px">
				  文件：
			  </td>
			  <td>
				  &nbsp;<asp:FileUpload ID="newsphoto" runat="server" onchange="CheckFile(this);" />
				  ...
				  <asp:Literal ID="Literal2" runat="server"></asp:Literal>
						...
						<tr>
							...
							<td>
								<asp:FileUpload ID="newsphoto3" runat="server" />
		...
		{ %><tr <%=ShowField("NewsPhoto") %>>
			  ...
			  <td>
				  &nbsp;<asp:FileUpload ID="newsphoto1" runat="server" />
...
<iframe id="Iframe1" src="SongEdit/editor.htm?id=txtContent&ReadCookie=0" frameborder="0" scrolling="no" width="100%" height="700"></iframe>--%>
		...
		{ %>
		   ...
		   { %>
									...
									<td align="center">
										<img alt="" height="60px" src="<%# CheckPic(Eval("text2").ToString())  %>" onload="AutoPhoto(100,60,this)" />
									</td>
									<%}
		   else
		   { %><td <%=ShowField("NewsPhoto") %> align="center">
